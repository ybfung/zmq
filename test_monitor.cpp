...
extern "C"
{
    ...
    static void *req_socket_monitor (void *ctx)
    {
	    ...
	    if (zmq_msg_recv (&msg, s, 0) == -1) break;
...
extern "C"
{
	...
	uint8_t *data = (uint8_t *) zmq_msg_data (&msg);
	printf("received from fd %d, content : %s\n",srcFd,(char*)data);
...
int main (void)
{
    ...
    rc = zmq_setsockopt (req, ZMQ_IDENTITY, "conn1", 6);
    assert (rc == 0);
    rc = zmq_connect (req, addr);
    assert (rc == 0);

    // 2nd REQ socket
    rc = zmq_setsockopt (req, ZMQ_IDENTITY, "conn2", 6);
    assert (rc == 0);
    req2 = zmq_socket (ctx, ZMQ_DEALER);
    assert (req2);
    ...
    //////////////////////////////////////////////////////////////////////////////////
    rc = pthread_create (&threads [3], NULL, rec_msg, rep);
    assert (rc == 0);

    char content[] = "1234567890";
    char content2[] = "ABCDEFGHIJK";
    rc = zmq_send (req, content, sizeof(content), 0);
    rc = zmq_send (req2, content2, sizeof(content), 0);
    rc = zmq_send (req, content, sizeof(content), 0);
